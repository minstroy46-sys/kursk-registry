...
index ccc37e5be625e90162ca50aa43d454ed3298c23f..901d6f56c964a9cef514bbdcd08565bdddd17565 100644
--- a/app.py
+++ b/app.py
@@ -17,71 +17,86 @@ st.set_page_config(page_title="Реестр объектов", layout="wide")
 # HELPERS
 # =============================
...
@@ -147,50 +162,66 @@ def try_parse_date(v) -> date | None:
             return None
         return dt.date()
     except Exception:
         return None
...
@@ -269,55 +300,79 @@ def normalize_schema(df: pd.DataFrame) -> pd.DataFrame:
...
@@ -350,50 +405,101 @@ def normalize_schema(df: pd.DataFrame) -> pd.DataFrame:
     ) else ""
...
+
 ...
 /* Prefer system dark */
@@ -473,61 +579,61 @@ html, body, [data-testid="stAppViewContainer"]{
 .hero-ministry{
...
+.card[data-accent="blue"]{ border-color: rgba(59,130,246,.45); box-shadow: 0 10px 22px var(--shadow); }
 ...
 .chip{
   ...
   font-size: 13px;
   color: var(--text);
@@ -615,80 +721,120 @@ html, body, [data-testid="stAppViewContainer"]{
 .section-title{
   font-weight: 900;
   color: var(--text);
   margin-bottom: 8px;
...
@@ -716,175 +862,172 @@ if APP_PASSWORD:
 raw = load_data()
 if raw.empty:
     st.error(
...
-    {btn_folder}
 ...
 """,
     ...
     with st.expander(exp_label, expanded=False):
         ...
         # Соглашение
@@ -914,35 +1057,40 @@ def render_card(row: pd.Series):
...
+        st.markdown(
+            '<div class="passport-collapse"><span>⬆️ Свернуть паспорт</span></div>',
+            unsafe_allow_html=True,
+        )
+
