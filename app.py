...
# =============================
# SAFE HTML RENDER (важно!)
# =============================
def html_block(s: str):
    """
    Главная защита от "всё стало кодом":
    - убираем отступы (dedent)
    - убираем пустые строки в начале/конце (strip)
    - рендерим как HTML
    """
...
def normalize_schema(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["name"] = df[col("name", "object_name", "наименование объекта", "объект")] if col(
        "name", "object_name", "наименование объекта", "объект"
    ) else ""
    ...
    out["work_flag"] = df[col("works_in_progress", "работы", "works")] if col(
        "works_in_progress", "работы", "works"
    ) else ""
    out["issues"] = df[col("issues", "проблемы", "проблемные вопросы")] if col(
        "issues", "проблемы", "проблемные вопросы"
    ) else ""
    out["updated_at"] = df[col("updated_at", "last_update", "обновлено")] if col(
        "updated_at", "last_update", "обновлено"
    ) else ""

    out["card_url"] = df[col("card_url_text", "card_url", "ссылка на карточку")] if col(
        "card_url_text", "card_url", "ссылка на карточку"
    ) else ""

    # Паспортные поля (если есть)
    out["state_program"] = df[col("state_program", "гп")] if col("state_program", "гп") else ""
    out["federal_project"] = df[col("federal_project", "фп")] if col("federal_project", "фп") else ""
    out["regional_program"] = df[col("regional_program", "рп")] if col("regional_program", "рп") else ""

    out["agreement"] = df[col("agreement", "соглашение")] if col("agreement", "соглашение") else ""
    ...
    out["capacity_seats"] = df[col("capacity_seats", "мощность")] if col("capacity_seats", "мощность") else ""
    out["area_m2"] = df[col("area_m2", "площадь")] if col("area_m2", "площадь") else ""
    out["target_deadline"] = df[col("target_deadline", "целевой срок")] if col("target_deadline", "целевой срок") else ""

    out["design"] = df[col("design", "псд")] if col("design", "псд") else ""
    ...
    out["expertise_conclusion"] = df[col("expertise_conclusion", "заключение")] if col("expertise_conclusion", "заключение") else ""
    ...
    out["contract"] = df[col("contract", "контракт")] if col("contract", "контракт") else ""
    ...
    out["contract_price"] = df[col("contract_price", "цена контракта")] if col("contract_price", "цена контракта") else ""
...
# =============================
# STYLES (светлые, читаемые на телефоне)
# =============================
...
label, .stMarkdown, .stCaption { color: var(--text) !important; }
div[data-testid="stCaptionContainer"] *{ color: rgba(15,23,42,.60) !important; }
...
/* Filters panel (чтобы не сливались) */
.filters-panel{
...
.card-title{ font-size: 20px; line-height: 1.18; font-weight: 900; margin: 0 0 12px 0; color: var(--text); }
...
.card-grid{ display:grid; grid-template-columns: 1fr 1fr; gap: 10px 18px; margin-bottom: 12px; }
...
.ico{ width: 22px; flex: 0 0 22px; display:flex; justify-content:center; margin-top: 1px; }
.ltxt{ color: var(--text); font-size: 14px; }
.ltxt b{ color: var(--text); }

.card-tags{ display:flex; gap:10px; flex-wrap:wrap; margin-bottom: 12px; }
.tag{
  display:inline-flex; align-items:center; gap: 8px;
  padding: 6px 10px; border-radius: 999px;
  border: 1px solid var(--chip-bd); background: var(--chip-bg);
...
.card-actions{ display:flex; gap:12px; margin: 10px 0 10px 0; }
.a-btn{
  flex: 1 1 0; display:flex; justify-content:center; align-items:center; gap: 8px;
...
.section{ padding: 12px; border-radius: 14px; border: 1px solid rgba(15,23,42,.10); background: rgba(255,255,255,.92); }
.section-title{ font-weight: 900; color: var(--text); margin-bottom: 8px; font-size: 14px; }
.row{ color: var(--text); font-size: 13.5px; line-height: 1.35; margin-bottom: 6px; }
.issue-box{
...
.collapse-center{ display:flex; justify-content:center; margin-top: 12px; }
</style>
""")
...
sectors = move_prochie_to_bottom(sorted([x for x in df["sector"].unique().tolist() if str(x).strip()]))
...
# FILTERS
# =============================
html_block('<div class="filters-panel">')
...
if q:
    ...
    filtered = filtered[filtered["_search_full"].astype(str).apply(lambda x: nq in x)]
...
def render_card(row: pd.Series):
    ...
    w_tag = "tag-red" if w_col == "red" else ("tag-green" if w_col == "green" else "tag-yellow" if w_col == "yellow" else "")

    u_col, u_txt = update_color(row.get("updated_at", ""))
    u_tag = "tag-red" if u_col == "red" else ("tag-green" if u_col == "green" else "tag-yellow" if u_col == "yellow" else "")

    s_tag = "tag-red" if accent == "red" else ("tag-green" if accent == "green" else "tag-yellow" if accent == "yellow" else "")
    ...
    # ВАЖНО: без отступов => не превратится в code-block
    html_block(f"""
...
""")
    ...
    if st.session_state[key_open]:
        html_block('<div class="passport-wrap">')
...
""")
        ...
        # Секции 2 колонки
        html_block('<div class="passport-grid">')
...
""")
        ...
        html_block("</div>")  # passport-grid

        html_block('<div class="collapse-center">')
        if st.button("▲", key=f"collapse_{key_open}"):
            st.session_state[key_open] = False
            st.rerun()
        html_block("</div>")

        html_block("</div>")  # passport-wrap

    html_block("</div>")  # card close
